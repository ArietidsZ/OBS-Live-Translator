...
mod tests {
    ...
    fn test_websocket_message_serialization() {
        let message = WebSocketMessage::Audio {
        ...
        match parsed {
            WebSocketMessage::Audio { data, sample_rate, timestamp } => {
    ...
    #[test]
    fn test_binary_audio_conversion() {
        let audio_samples = vec![0.5f32, -0.3f32, 1.0f32];
        ...
        // Convert back
        let converted: Vec<f32> = binary_data
            .chunks(4)
            .map(|chunk| {
                let bytes = [chunk[0], chunk[1], chunk[2], chunk[3]];
                f32::from_le_bytes(bytes)
        ...
        assert_eq!(converted, audio_samples);
    }
}